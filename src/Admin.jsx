...
function Admin() {
  ...
  const navigate = useNavigate(); // Aquí utilizas useNavigate
  const location = useLocation();
  const { state } = location || {};
  const { username, password, tipo } = state || {};
  console.log('Datos recibidos en Admin:', username, password, tipo);
  ...
  const redirectToRepuestos = () => {
    navigate("/repuestos", { state: { username: username, password: password, tipo: tipo } });
  };
  ...
  return (
    <div>
      <div className="admin-container">
        ...
        <button onClick={redirectToRepuestos}>Repuestos</button>
