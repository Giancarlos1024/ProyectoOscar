...
// Endpoint para obtener un repuesto por su ID
app.get('/repuestos/:id', async (req, res) => {
  const { id } = req.params;
  try {
    await sql.connect(config);
    const result = await sql.query(`SELECT * FROM Repuestos WHERE id = ${id}`);
    if (result.recordset.length === 0) {
...
// Endpoint para agregar un nuevo repuesto
app.post('/repuestos', async (req, res) => {
  const { tipo, descripcion, cantidad, precio } = req.body;
  try {
    await sql.connect(config);
    await sql.query(`
      INSERT INTO Repuestos (tipo, descripcion, cantidad, precio)
      VALUES ('${tipo}', '${descripcion}', ${cantidad}, ${precio})
    `);
    res.status(201).send('Repuesto creado exitosamente');
  } catch (error) {
...
// Endpoint para actualizar un repuesto existente
app.put('/repuestos/:id', async (req, res) => {
  ...
  try {
    ...
    await sql.query(`
      UPDATE Repuestos
      SET tipo = '${tipo}', descripcion = '${descripcion}', cantidad = ${cantidad}, precio = ${precio}
      WHERE id = ${id}
    `);
    res.status(200).send('Repuesto actualizado exitosamente');
  } catch (error) {
...
// Endpoint para eliminar un repuesto
app.delete('/repuestos/:id', async (req, res) => {
  const { id } = req.params;
  try {
    await sql.connect(config);
    await sql.query(`DELETE FROM Repuestos WHERE id = ${id}`);
    res.status(200).send('Repuesto eliminado exitosamente');
  } catch (error) {
